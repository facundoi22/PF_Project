...
class ClasePadre
{
    ...
    {
        $query = "SELECT COLUMN_NAME, COLUMN_KEY FROM `information_schema`.`COLUMNS` WHERE TABLE_SCHEMA = 'dw4_proximafecha' AND TABLE_NAME  = ? ";
        $stmt = DBConnection::getStatement($query);
    ...
    {
        if(!is_array($keys)) {
            throw new ParametroInvalidoException("El parámetro no es un array");
        };
